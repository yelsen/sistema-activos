...
public class PermisosDataLoader extends AbstractDataLoader {
    ...
    protected boolean shouldLoad() {
        // Calcular permisos esperados aplicando la regla: GENERAR solo en módulo Reportes
        List<ModuloSistema> modulos = moduloSistemaRepository.findAll();
        List<Accion> acciones = accionRepository.findAll();

        long permisosEsperados = 0;
        for (ModuloSistema modulo : modulos) {
            for (Accion accion : acciones) {
                // Saltar GENERAR si el módulo no es Reportes
                if ("GENERAR".equalsIgnoreCase(accion.getCodigoAccion())
                        && !"Reportes".equalsIgnoreCase(modulo.getNombreModulo())) {
                    continue;
                }
                permisosEsperados++;
            }
